...
import { bulkCreateGrounds } from './actions'
...
function validateRow(r: Record<string, string>): string[] {
  ...
  if (!r.club || !r.club.trim()) issues.push('Missing club')
  if (!r.slug || !r.slug.trim()) issues.push('Missing slug')
  if (!r.country || !r.country.trim()) issues.push('Missing country')
  if (!r.website || !r.website.trim()) issues.push('Missing website')
...
export default function BulkUploadForm() {
  ...
  function handlePreview() {
    ...
    const required = ['name', 'club', 'slug', 'country', 'website', 'latitude', 'longitude']
    ...
    const rows: ParsedRow[] = grid.slice(1).map(line => {
      ...
      return {
        name: obj.name, club: obj.club, slug: obj.slug, country: obj.country,
        website: obj.website || '', latitude: obj.latitude, longitude: obj.longitude, issues,
      }
    })
  ...
  async function handleConfirm() {
    ...
    const result = await bulkCreateGrounds(validRows)

    if (result.success) {
      let msg = `Added ${result.inserted} new ground${result.inserted === 1 ? '' : 's'}.`
      if (result.enriched.length > 0) {
        msg += ` Enriched ${result.enriched.length} existing ground${result.enriched.length === 1 ? '' : 's'}: ` +
      ...
      {stage === 'input' && (
        <>
          <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', marginBottom: '12px', lineHeight: 1.6 }}>
            Paste CSV with these columns (any order): <code style={{ color: 'rgba(255,255,255,0.7)' }}>name, club, slug, country, website, latitude, longitude</code>
          </p>
          <textarea
            value={csvText}
            onChange={e => setCsvText(e.target.value)}
            placeholder="name,club,slug,country,website,latitude,longitude"
            style={{
        ...
        <>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: 'rgba(255,255,255,0.05)' }}>
                  {['#', 'Name', 'Club', 'Slug', 'Country', 'Lat/Long', 'Issues'].map(h => (
                ...
                {parsed.map((r, i) => {
                  ...
                  return (
                    <tr key={i} style={{ background: hasError ? 'rgba(239,68,68,0.08)' : 'transparent', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                      ...
                      <td style={{ padding: '8px 10px', color: 'rgba(255,255,255,0.6)' }}>{r.club}</td>
                      <td style={{ padding: '8px 10px', color: 'rgba(255,255,255,0.6)' }}>{r.slug}</td>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            <button onClick={handleConfirm} disabled={validCount === 0} style={{
              ...
              Add {validCount} ground{validCount === 1 ? '' : 's'}
            </button>
          </div>
        </>
      )}
